...
**Phase 0 and Phase 1 are complete. Phase 2 (Hadoop stack) is written but not run anywhere.** The `lite` profile (`docker compose --profile lite up`) is the only way this project has actually been exercised end to end — it still runs against the original Postgres/Kafka mock stack described below. `docker-compose.yml`'s `hadoop` profile now defines HDFS, YARN, Hive, and a Spark transform job (see [`docs/hadoop-stack.md`](docs/hadoop-stack.md)), but **none of it has been started, pulled, or run** — the environment this was built in blocks Docker image pulls entirely. Config-validated (`docker compose config`) and, for the one piece that could be tested without the cluster (the Spark transform's actual logic), genuinely run against a local PySpark session — everything else is written from documented, standard configuration, not confirmed against a live cluster. Nothing in this README or `docs/hadoop-stack.md` should be read as claiming the Hadoop stack has been run.
